	...
	public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
		...
		if (ACTION_GENID.equals(uri.getPath())) {
			...
			if (log.isTraceEnabled())
				log.trace("Generated id: " + sn);

			sbContent.append(sn);
		} else if (ACTION_EXPID.equals(uri.getPath())) {
			SerialNumber sn = snService.inverseSerialNumber(id);

			if (log.isTraceEnabled())
				log.trace("Explained id: " + sn);
			...
			if (time == -1 || seq == -1)
				...
				if (type == -1) {
					...
					if (machine == -1) {
						madeId = snService.generateSerialNumber(time, seq);
					} else {
						madeId = snService.generateSerialNumber(machine, time, seq);
					}

				} else {
					madeId = snService.generateSerialNumber(type, machine, time, seq);
				}
			} else {
				madeId = snService.generateSerialNumber(version, type, machine, time, seq);
			}
			...
			sbContent.append("\r\n");
			sbContent
					.append("    Example 4: http://ip:port/makeid?version=?&type=?&genmethod=?&machine=?&time=?&seq=?");
		...
		FullHttpResponse response = new DefaultFullHttpResponse(HTTP_1_1, OK,
				Unpooled.wrappedBuffer(sbContent.toString().getBytes(Charset.forName("UTF-8"))));
